...
public class MaintenanceDao extends DBConnector {
	...
	public ArrayList<Maintenance> getMaintenanceByUser(int userId, int vehicleId) throws SQLException {
		String sql = "Select m.maintenance_id, m.mileage, s.service, s.service_discription FROM maintenance m, service_lookup s WHERE user_id = "
				+ userId
				+ " AND vehicle_id = "
				+ vehicleId
		...
		if (rs != null) {
			while (rs.next()) {
				...
				Maintenance m = new Maintenance(maintenance, mileage, service, discription);
				records.add(m);
